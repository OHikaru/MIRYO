// REVISED: マルチAI＋RAG前提でAppContext.aiChatを呼ぶ。根拠表示・人エスカレーションのUIを整備。
import React, { useEffect, useRef, useState } from 'react';
import { Bot, Send, AlertTriangle, ExternalLink, Brain, Loader, CheckCircle, XCircle } from 'lucide-react';
...
const AIAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    { role: 'system', content: 'あなたは医療向けAIアシスタント。アップロードされたソースに基づき、根拠URIを付与して回答。危険/不確実は人へ。' }
  ]);
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {messages.filter(m => m.role!=='system').map((m, i) => (
          <div key={i} className={`max-w-3xl ${m.role==='user' ? 'ml-auto' : ''}`}>
            <div className={`p-3 rounded-lg border ${m.role==='user' ? 'bg-blue-50 border-blue-200' : 'bg-white'}`}>
              ...
              {m.action && m.action !== 'continue_ai' && (
                <div className={`mt-2 text-xs ${m.action==='handoff_human'?'text-orange-600':'text-red-600'}`}>
                  推奨アクション: {m.action==='handoff_human'?'人へのエスカレーション':'緊急連絡'}
                </div>
              )}
            </div>
          </div>
      ...
      <div className="p-3 border-t bg-white">
        ...
        <div className="mt-1 text-xs text-gray-500">※ 根拠（出典URI）付きで回答します。不確実・危険時は人へエスカレーション。</div>
      </div>
    </div>
  );
};
